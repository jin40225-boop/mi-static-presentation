...
export const BranchingScenario: React.FC<BranchingScenarioProps> = ({
  ...
  const expressionLabel =
    currentNode.expression === 'angry'
      ? '防衛'
      : currentNode.expression === 'sad'
        ? '低落'
        : currentNode.expression === 'happy'
          ? '鬆動'
          : '觀望';
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6">
      <div className="glass-panel p-4 rounded-2xl flex items-center gap-4">
        ...
        <div className="text-2xl font-bold text-clay-orange w-12 text-center">{Math.round(trustScore)}</div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-1 flex flex-col items-center justify-center glass-panel p-6 rounded-3xl min-h-[300px]">
          ...
          >
            ...
            <div className="text-center text-xs text-warm-stone uppercase tracking-widest mt-1">{expressionLabel}</div>
          </motion.div>
        </div>
        ...
        <div className="md:col-span-2 space-y-4">
          ...
          >
            ...
            <p className="text-xl leading-relaxed italic text-warm-charcoal">{currentNode.text}</p>
          </motion.div>
                    ...
                    <div className="space-y-4">
                      <div className="p-6 bg-green-50 border border-green-200 rounded-2xl text-green-800">
                        <h4 className="font-bold mb-2 flex items-center gap-2">
                          <CheckCircle2 size={20} /> 情境完成
                        </h4>
                        <p>你已完成這段對話示範，最終互信分數為 {trustScore}。</p>
                      </div>
                      <button
                        onClick={reset}
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-center mb-2">
                        <h4 className="font-bold text-lg">回應解析</h4>
                        {lastFeedback?.skill && (
                      ...
                      <p className="text-warm-stone leading-relaxed mb-4">{lastFeedback?.text}</p>
                      <div className="flex items-center gap-2 text-sm font-bold">
                        <span>互信變化</span>
                        <span className={cn((lastFeedback?.trustChange || 0) > 0 ? 'text-green-600' : 'text-red-600')}>
                          {(lastFeedback?.trustChange || 0) > 0 ? '+' : ''}
                          {(lastFeedback?.trustChange || 0) * 10}
                        </span>
                      </div>
                    </div>
                  </div>
